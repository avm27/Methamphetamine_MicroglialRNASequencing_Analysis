...
colData <- read.csv("data/sample_info_final.csv", sep = ",", row.names = 1)
colData
countData <-as.matrix(read.csv("data/MIVSA_gene_count_matrix.csv", row.names = "gene_id"))
...
write_csv((as.data.frame(results_sig)), "output/Sig_Res_Maintenace_vs_Saline_Final.csv")
write_csv((as.data.frame(results_sig2)), "output/Sig_Res_Abstinence_vs_Saline_Final.csv")
write_csv((as.data.frame(results_sig3)), "output/Sig_Res_Abstinence_vs_Maintenance_Final.csv")
```
...
write.table(
  ...
  file = "data/MIVSA_normalized_counts_DESeq2.txt",
  sep = "\t",
  quote = F,
  col.names = NA
)
...
#pdf(file = "output/PCA_MIVSA_Final.pdf", width = 12, height = 12);
...
write.table(
  ...
  file = "output/Gene_Matrix_EntrezIDs_MNvsSAL.txt",
  sep = "\t",
  quote = F,
  col.names = NA
)
write.table(
  ...
  file = "output/Gene_Matrix_EntrezIDs_ABSvsSAL.txt",
  sep = "\t",
  quote = F,
  col.names = NA
)
write.table(
  ...
  file = "output/Gene_Matrix_EntrezIDs_ABSvsMN.txt",
  sep = "\t",
  quote = F,
  col.names = NA
)
...
data <- read.csv("data/Filtered_GO_Combined_Final.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
...
barplots <- read.csv("data/Comparison_GO_GeneID_barplots.csv")
```
...
pdf(file = "output/GOBarplots_MIVSA.pdf", width = 25, height = 14)
